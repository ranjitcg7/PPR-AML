...
class ExtractConnections:
    ...
    def fetch_extracted_IH_payload(self):
    ...
    def get_view_id(self, name='selected_view'):
    ...
    def get_external_interface_ids(self, view_ID, data = None,):
        # If data is None, initialize it to payload
        if data is None:
            data = self.fetch_extracted_IH_payload()

        # Initialize an empty list to store the external interface ids
        external_interface_ids = []

        # If the data is a dictionary and it has 'ID' and 'ExternalInterface' keys
        if isinstance(data, dict):
            if 'ID' in data and data['ID'] == view_ID and 'ExternalInterface' in data:
                # Add all the 'ID' values in 'ExternalInterface' to the list
                for ext_interface in data['ExternalInterface']:
                    if 'ID' in ext_interface:
                        external_interface_ids.append(ext_interface['ID'])

            # Recurse into the values of the dictionary
            for value in data.values():
                external_interface_ids.extend(self.get_external_interface_ids(value))

        # If the data is a list
        elif isinstance(data, list):
            # Recurse into the items of the list
            for item in data:
                external_interface_ids.extend(self.get_external_interface_ids(item))
...
view_ID = connections.get_view_id()

id = connections.get_external_interface_ids(view_ID)
