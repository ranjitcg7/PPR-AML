...
class IntegrateViews:
    ...
    def fetch_ea_cci(self):
        doc_ref = self.db.collection("utils").document("ea_cci")
        doc = doc_ref.get()
        if doc.exists:
            return doc.to_dict()
        else:
    ...
    def integrate_views(self, raw_views, ea_cci):
        id_to_ie = {}
        ...
        for view_data in raw_views:
            for entry in view_data['IH_views']:
                ...
                base_system_unit_path = entry['RefBaseSystemUnitPath']
                if id_value in id_to_ie:
                    ...
                    existing_ie.ref_base_system_unit_path = base_system_unit_path[0] # check this line
                else:
                    ie = InternalElementType(name=name_value, id=str(uuid.uuid4()))
                    ie.internal_element.append(view_value)
                    ie.ref_base_system_unit_path = base_system_unit_path[0] # check this line
        ...
        for sucl in ea_cci['SystemUnitClassLib']:
            self.new_file.system_unit_class_lib.append(sucl)

        for rcl in ea_cci['RoleClassLib']:
            self.new_file.role_class_lib.append(rcl)

        for ifcl in ea_cci['InterfaceClassLib']:
            self.new_file.interface_class_lib.append(ifcl)

        for atl in ea_cci['AttributeTypeLib']:
            self.new_file.attribute_type_lib.append(atl)
    ...
    def execute(self):
        ...
        ea_cci = self.fetch_ea_cci()
        self.integrate_views(raw_views, ea_cci)
